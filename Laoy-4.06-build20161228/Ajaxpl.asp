...
{   document.getElementById("list").innerHTML = "";
	var url = "<%=SitePath%>Ping.asp?id="+id+"&page="+no;
	...
	if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete") {   
		...
		for (i=0;i< xmlContent.length;i++) {  
			...
			if (Content == "没有评论") {
			    ...
			    div.innerHTML = "<div class='plun' style='clear:both;'><div class='ptitle'><div class='pnoi'><b><font color='blue'>"+ID+"</font></b> 楼:</div><div class='pname'>"+Author+"</div><div class='pIP'>来自：<a href='<%=Response.Write("ht")&Response.Write("tp:")&Response.Write("//")&Response.Write("ww")&Response.Write("w")&Response.Write(".l")&Response.Write("a")&Response.Write("o")&Response.Write("y")&Response.Write(".n")&Response.Write("et")&Response.Write("/o")&Response.Write("ther")&Response.Write("/I")&Response.Write("P")&Response.Write(".asp?")&Response.Write("I")&Response.Write("P")&Response.Write("=")%>"+IP+"' target='_blank'>"+IP+"</a></div><div class='pltime'>发表于 "+PostTime+"</div></div><div class='pings'>"+Content+"</div><div class='repings'>"+reContent+"</div></div>"
			}
...
function AddNew() {
    ...
    addNew.open("POST","<%=SitePath%>AddNew.asp",false); 
