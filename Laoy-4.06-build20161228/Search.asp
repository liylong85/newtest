...
<%=Menu%><div class="mw">
	<div class="dh">
		<%=search%>您现在的位置：<a href="<%=sitepath%>">首页</a> >> 搜索"<%=KeyWord%>"
    </div>
	<div id="nw_left">
		<div id="web2l">
        	<h6>站内搜索</h6>
...
for i=1 to mypage.pagesize
    if not rs.eof then 
%>
				<li style="border-bottom:1px dashed #ccc;"><%if rs("Images")<>"" and artlist=0 then%><div style="float:left;margin:5px 5px 0 5px;"><a href="<%=apath(rs("ID"),0)%>" target="_blank"><img src="<%=rs("Images")%>" style="width:90px;height:90px;" alt="<%=rs("title")%>"/></a></div><%end if%><%If rs("IsTop")=1 then Response.Write("<font color=red>[顶]</font>") end if%><a href="<%=apath(rs("ID"),0)%>" target="_blank"><%if rs("TitleFontColor")<>"" then Response.Write("<font style=""color:"&rs("TitleFontColor")&""">"&replacecolor(rs("Title"))&"</font>") else Response.Write(""&replacecolor(rs("Title"))&"") end if%></a>
				<span style="color:#AAA;font-size:12px;"><%=FormatDate(rs("DateAndTime"),11,1)%> 点击：<%=rs("Hits")%> 评论:<%=Mydb("Select Count([ID]) From ["&tbname&"_Pl] Where yn=1 And ArticleID="&rs("id")&"",1)(0)%></span></li>
				<%If artlist=0 then%>
